...
#include "../interface/RooDijetFisherPolExt5Pdf.h"
...
ClassImp(RooDijetFisherPolExt5Pdf)
//---------------------------------------------------------------------------
RooDijetFisherPolExt5Pdf::RooDijetFisherPolExt5Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
				   RooAbsReal& _p1, 
                   RooAbsReal& _p2, 
				   RooAbsReal& _p3, 
...
//---------------------------------------------------------------------------
RooDijetFisherPolExt5Pdf::RooDijetFisherPolExt5Pdf(const RooDijetFisherPolExt5Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherPolExt5Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherPolExt5Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherPolExt5Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherPolExt5Pdf::evaluate() const
{
  ...
  DijetFisherPolExt5Function func;
  double params[5];
  params[0] = sqrts;    
  params[1] = p1;
  params[2] = p2;       
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherPolExt5Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherPolExt5Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[9] = p9;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherPolExt5Pdf: integration code is not correct" << endl;
