...
#include "../interface/RooDijetFisherNewAlt4Pdf.h"
...
ClassImp(RooDijetFisherNewAlt4Pdf)
//---------------------------------------------------------------------------
RooDijetFisherNewAlt4Pdf::RooDijetFisherNewAlt4Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,
				   RooAbsReal& _p1,
                   RooAbsReal& _p2,
				   RooAbsReal& _p3,
...
//---------------------------------------------------------------------------
RooDijetFisherNewAlt4Pdf::RooDijetFisherNewAlt4Pdf(const RooDijetFisherNewAlt4Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt4Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt4Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt4Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherNewAlt4Pdf::evaluate() const
{
  ...
  DijetFisherNewAlt4Function func;
  double params[4];
  params[0] = sqrts;
  params[1] = p1;
  params[2] = p2;
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherNewAlt4Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherNewAlt4Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[6] = p6;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherNewAlt4Pdf: integration code is not correct" << endl;
