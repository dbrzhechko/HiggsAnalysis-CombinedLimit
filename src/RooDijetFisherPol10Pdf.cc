...
#include "../interface/RooDijetFisherPol10Pdf.h"
...
ClassImp(RooDijetFisherPol10Pdf)
//---------------------------------------------------------------------------
RooDijetFisherPol10Pdf::RooDijetFisherPol10Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
				   RooAbsReal& _p1, 
                   RooAbsReal& _p2, 
				   RooAbsReal& _p3, 
...
//---------------------------------------------------------------------------
RooDijetFisherPol10Pdf::RooDijetFisherPol10Pdf(const RooDijetFisherPol10Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherPol10Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherPol10Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherPol10Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherPol10Pdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherPol10Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherPol10Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherPol10Pdf: integration code is not correct" << endl;
