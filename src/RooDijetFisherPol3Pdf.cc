...
#include "../interface/RooDijetFisherPol3Pdf.h"
...
ClassImp(RooDijetFisherPol3Pdf)
//---------------------------------------------------------------------------
RooDijetFisherPol3Pdf::RooDijetFisherPol3Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
				   RooAbsReal& _p1, 
                   RooAbsReal& _p2, 
//				   RooAbsReal& _p3, 
...
//---------------------------------------------------------------------------
RooDijetFisherPol3Pdf::RooDijetFisherPol3Pdf(const RooDijetFisherPol3Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherPol3Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherPol3Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherPol3Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherPol3Pdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherPol3Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherPol3Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[9] = p9;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherPol3Pdf: integration code is not correct" << endl;
