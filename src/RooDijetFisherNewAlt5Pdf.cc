...
#include "../interface/RooDijetFisherNewAlt5Pdf.h"
...
ClassImp(RooDijetFisherNewAlt5Pdf)
//---------------------------------------------------------------------------
RooDijetFisherNewAlt5Pdf::RooDijetFisherNewAlt5Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,
				   RooAbsReal& _p1,
                   RooAbsReal& _p2,
				   RooAbsReal& _p3,
...
//---------------------------------------------------------------------------
RooDijetFisherNewAlt5Pdf::RooDijetFisherNewAlt5Pdf(const RooDijetFisherNewAlt5Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt5Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt5Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherNewAlt5Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherNewAlt5Pdf::evaluate() const
{
  ...
  DijetFisherNewAlt5Function func;
  double params[5];
  params[0] = sqrts;
  params[1] = p1;
  params[2] = p2;
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherNewAlt5Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherNewAlt5Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[6] = p6;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherNewAlt5Pdf: integration code is not correct" << endl;
