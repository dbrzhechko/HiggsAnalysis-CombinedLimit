...
#include "../interface/RooDijetFisherPol5Pdf.h"
...
ClassImp(RooDijetFisherPol5Pdf)
//---------------------------------------------------------------------------
RooDijetFisherPol5Pdf::RooDijetFisherPol5Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
				   RooAbsReal& _p1, 
                   RooAbsReal& _p2, 
				   RooAbsReal& _p3, 
...
//---------------------------------------------------------------------------
RooDijetFisherPol5Pdf::RooDijetFisherPol5Pdf(const RooDijetFisherPol5Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherPol5Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherPol5Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherPol5Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherPol5Pdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherPol5Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherPol5Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[9] = p9;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherPol5Pdf: integration code is not correct" << endl;
