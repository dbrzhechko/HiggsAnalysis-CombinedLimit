...
#include "../interface/RooDijetFisherAlt6Pdf.h"
...
ClassImp(RooDijetFisherAlt6Pdf)
//---------------------------------------------------------------------------
RooDijetFisherAlt6Pdf::RooDijetFisherAlt6Pdf(const char *name, const char *title,
				   RooAbsReal& _th1x,  
				   RooAbsReal& _p1, 
                   RooAbsReal& _p2, 
				   RooAbsReal& _p3, 
...
//---------------------------------------------------------------------------
RooDijetFisherAlt6Pdf::RooDijetFisherAlt6Pdf(const RooDijetFisherAlt6Pdf& other, const char* name) :
...
//---------------------------------------------------------------------------
void RooDijetFisherAlt6Pdf::setTH1Binning(TH1* _Hnominal){
...
//---------------------------------------------------------------------------
void RooDijetFisherAlt6Pdf::setRelTol(double _relTol){
  relTol = _relTol;
}
//---------------------------------------------------------------------------
void RooDijetFisherAlt6Pdf::setAbsTol(double _absTol){
  absTol = _absTol;
}
//---------------------------------------------------------------------------
Double_t RooDijetFisherAlt6Pdf::evaluate() const
{
...
// //---------------------------------------------------------------------------
Int_t RooDijetFisherAlt6Pdf::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* rangeName) const{
  if (matchArgs(allVars, analVars, th1x)) return 1;
  return 0;
}

// //---------------------------------------------------------------------------
Double_t RooDijetFisherAlt6Pdf::analyticalIntegral(Int_t code, const char* rangeName) const{
...
//   params[9] = p9;
  ...
  ig.SetFunction(func,false);
   ...
   if (code==1 && iBinMin<=0 && iBinMax>=xBins){
     ...
     cout << "WARNING IN RooDijetFisherAlt6Pdf: integration code is not correct" << endl;
