...
public:
   ...
   {
     double pdf = exp(log(TMath::Max(1E-9,p[2] * x/p[0] - 1)) + p[3]*x/p[0] + p[4]*pow(x/p[0],2) + p[5]*pow(x/p[0],3) + p[6]*pow(x/p[0],4))/pow(x/p[0],p[1]);
     return pdf;
   }
   double DoEval(double x) const
   {
     double pdf = exp(log(TMath::Max(1E-9,pars[2] * x/pars[0] - 1)) + pars[3]*x/pars[0] + pars[4]*pow(x/pars[0],2) + pars[5]*pow(x/pars[0],3) + pars[6]*pow(x/pars[0],4))/pow(x/pars[0],pars[1]);
     return pdf;
   }
